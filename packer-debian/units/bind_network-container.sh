...
ExecStart=/usr/bin/docker run --rm --name=bind_network -v chef-secret-volume:/etc/chef --net=brlan --ip 192.168.63.251 -v /etc/resolv.conf:/etc/resolv.conf:ro --cap-add=NET_ADMIN --device /dev/net/tun randomcoww/chef-client:entrypoint -o 'role[vpn_network_bind]'
