...
ExecStart=/usr/bin/docker run --rm --name=bind_network --net=docker1 --ip 172.20.0.4 -v /etc/resolv.conf:/etc/resolv.conf:ro --cap-add=NET_ADMIN --device /dev/net/tun -v chef-secret-volume:/etc/chef randomcoww/chef-client:entrypoint -o openvpn_proxy::run
