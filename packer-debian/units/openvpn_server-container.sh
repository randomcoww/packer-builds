...
ExecStart=/usr/bin/docker run --rm --name openvpn_server -v chef-secret-volume:/etc/chef --cap-add=NET_ADMIN --net host --device /dev/net/tun randomcoww/chef-client:entrypoint -o 'role[openvpn_server]'
